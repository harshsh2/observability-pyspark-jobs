...
def on_issue_validations(input_data):
    scope = payload_utils["get_json_path"](input_data["payload"], "$")
    sub_results = []
    valid = True

    for on_issue_validations_obj in scope:
        on_issue_validations_obj["_EXTERNAL"] = input_data["external_data"]
        ...
        def REQUIRED_MESSAGE_STATUS(input_data):
            ...
            for REQUIRED_MESSAGE_STATUS_obj in scope:
                ...
                enumList = ["OPEN","CLOSED","PROCESSING","NEED_MORE_INFO","RESOLVED","INFO_PROVIDED"]
          ...
          - **condition REQUIRED_MESSAGE_STATUS.2**: every element of $.message.issue.status must be in ["OPEN", "CLOSED", "PROCESSING", "NEED_MORE_INFO", "RESOLVED", "INFO_PROVIDED"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_STATUS","attr":"$.message.issue.status","_RETURN_":"attr are present && attr all in enumList","enumList":["OPEN","CLOSED","PROCESSING","NEED_MORE_INFO","RESOLVED","INFO_PROVIDED"]}
        """
                        }
                    }]
        ...
        def VALID_ENUM_MESSAGE_STATUS(input_data):
            ...
            for VALID_ENUM_MESSAGE_STATUS_obj in scope:
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition VALID_ENUM_MESSAGE_STATUS**: every element of $.message.issue.status must be in ["OPEN", "CLOSED", "PROCESSING", "RESOLVED", "NEED_MORE_INFO", "INFO_PROVIDED"]
        	...
        	> - **condition B**: $.message.issue.status must **not** be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"VALID_ENUM_MESSAGE_STATUS","enumList":["OPEN","CLOSED","PROCESSING","RESOLVED","NEED_MORE_INFO","INFO_PROVIDED"],"enumPath":"$.message.issue.status","_CONTINUE_":"!(enumPath are present)","_RETURN_":"enumPath all in enumList"}
        """
                        }
                    }]
...
def on_issue(input_data):
    ...
    if input_data["config"].get("only_invalid") is True:
        ...
        if len(res) == 0:
            target_success = next((r for r in total_results if r["test_name"] == "on_issue_validations"), None)
