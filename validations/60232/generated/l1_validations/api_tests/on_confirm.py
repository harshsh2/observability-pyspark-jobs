    ...
    for on_confirm_validations_obj in scope:
        ...
        def REQUIRED_MESSAGE_NAME_44(input_data):
            ...
            for REQUIRED_MESSAGE_NAME_44_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_44_obj, "$.message.order.fulfillments[*].end.person.name")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_NAME_44**: $.message.order.fulfillments[*].end.person.name must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_NAME_44","attr":"$.message.order.fulfillments[*].end.person.name","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_GPS_45(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_GPS_45_obj in scope:
                REQUIRED_MESSAGE_GPS_45_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_GPS_45_obj, "$.message.order.fulfillments[*].end.location.gps")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_GPS_45_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_GPS_45_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_GPS_45",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_GPS_45**: $.message.order.fulfillments[*].end.location.gps must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_GPS_45","attr":"$.message.order.fulfillments[*].end.location.gps","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_NAME_46(input_data):
            ...
            for REQUIRED_MESSAGE_NAME_46_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_46_obj, "$.message.order.fulfillments[*].end.location.address.name")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_NAME_46**: $.message.order.fulfillments[*].end.location.address.name must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_NAME_46","attr":"$.message.order.fulfillments[*].end.location.address.name","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_BUILDING_47(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_BUILDING_47_obj in scope:
                REQUIRED_MESSAGE_BUILDING_47_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_BUILDING_47_obj, "$.message.order.fulfillments[*].end.location.address.building")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_BUILDING_47_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_BUILDING_47_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_BUILDING_47",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_BUILDING_47**: $.message.order.fulfillments[*].end.location.address.building must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_BUILDING_47","attr":"$.message.order.fulfillments[*].end.location.address.building","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_LOCALITY_48(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_LOCALITY_48_obj in scope:
                REQUIRED_MESSAGE_LOCALITY_48_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_LOCALITY_48_obj, "$.message.order.fulfillments[*].end.location.address.locality")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_LOCALITY_48_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_LOCALITY_48_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_LOCALITY_48",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_LOCALITY_48**: $.message.order.fulfillments[*].end.location.address.locality must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_LOCALITY_48","attr":"$.message.order.fulfillments[*].end.location.address.locality","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_CITY_49(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_CITY_49_obj in scope:
                REQUIRED_MESSAGE_CITY_49_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_CITY_49_obj, "$.message.order.fulfillments[*].end.location.address.city")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_CITY_49_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_CITY_49_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_CITY_49",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_CITY_49**: $.message.order.fulfillments[*].end.location.address.city must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_CITY_49","attr":"$.message.order.fulfillments[*].end.location.address.city","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_STATE_50(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_STATE_50_obj in scope:
                REQUIRED_MESSAGE_STATE_50_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_STATE_50_obj, "$.message.order.fulfillments[*].end.location.address.state")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_STATE_50_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_STATE_50_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_STATE_50",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_STATE_50**: $.message.order.fulfillments[*].end.location.address.state must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_STATE_50","attr":"$.message.order.fulfillments[*].end.location.address.state","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_COUNTRY_51(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_COUNTRY_51_obj in scope:
                REQUIRED_MESSAGE_COUNTRY_51_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_COUNTRY_51_obj, "$.message.order.fulfillments[*].end.location.address.country")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_COUNTRY_51_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_COUNTRY_51_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_COUNTRY_51",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_COUNTRY_51**: $.message.order.fulfillments[*].end.location.address.country must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_COUNTRY_51","attr":"$.message.order.fulfillments[*].end.location.address.country","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_AREA_CODE_52(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_AREA_CODE_52_obj in scope:
                REQUIRED_MESSAGE_AREA_CODE_52_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_AREA_CODE_52_obj, "$.message.order.fulfillments[*].end.location.address.area_code")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_AREA_CODE_52_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_AREA_CODE_52_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_AREA_CODE_52",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_AREA_CODE_52**: $.message.order.fulfillments[*].end.location.address.area_code must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_AREA_CODE_52","attr":"$.message.order.fulfillments[*].end.location.address.area_code","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_PHONE_53(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_PHONE_53_obj in scope:
                REQUIRED_MESSAGE_PHONE_53_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_PHONE_53_obj, "$.message.order.fulfillments[*].end.contact.phone")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_PHONE_53_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_PHONE_53_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_PHONE_53",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_PHONE_53**: $.message.order.fulfillments[*].end.contact.phone must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_PHONE_53","attr":"$.message.order.fulfillments[*].end.contact.phone","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_NAME_54(input_data):
            ...
            for REQUIRED_MESSAGE_NAME_54_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_54_obj, "$.message.order.billing.name")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_NAME_54**: $.message.order.billing.name must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_NAME_54","attr":"$.message.order.billing.name","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_NAME_55(input_data):
            ...
            for REQUIRED_MESSAGE_NAME_55_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_55_obj, "$.message.order.billing.address.name")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_NAME_55**: $.message.order.billing.address.name must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_NAME_55","attr":"$.message.order.billing.address.name","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_BUILDING_56(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_BUILDING_56_obj in scope:
                REQUIRED_MESSAGE_BUILDING_56_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_BUILDING_56_obj, "$.message.order.billing.address.building")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_BUILDING_56_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_BUILDING_56",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_BUILDING_56**: $.message.order.billing.address.building must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_BUILDING_56","attr":"$.message.order.billing.address.building","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_LOCALITY_57(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_LOCALITY_57_obj in scope:
                REQUIRED_MESSAGE_LOCALITY_57_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_LOCALITY_57_obj, "$.message.order.billing.address.locality")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_LOCALITY_57_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_LOCALITY_57",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_LOCALITY_57**: $.message.order.billing.address.locality must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_LOCALITY_57","attr":"$.message.order.billing.address.locality","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_CITY_58(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_CITY_58_obj in scope:
                REQUIRED_MESSAGE_CITY_58_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_CITY_58_obj, "$.message.order.billing.address.city")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_CITY_58_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_CITY_58",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_CITY_58**: $.message.order.billing.address.city must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_CITY_58","attr":"$.message.order.billing.address.city","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_STATE_59(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_STATE_59_obj in scope:
                REQUIRED_MESSAGE_STATE_59_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_STATE_59_obj, "$.message.order.billing.address.state")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_STATE_59_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_STATE_59",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_STATE_59**: $.message.order.billing.address.state must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_STATE_59","attr":"$.message.order.billing.address.state","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_COUNTRY_60(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_COUNTRY_60_obj in scope:
                REQUIRED_MESSAGE_COUNTRY_60_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_COUNTRY_60_obj, "$.message.order.billing.address.country")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_COUNTRY_60_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_COUNTRY_60",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_COUNTRY_60**: $.message.order.billing.address.country must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_COUNTRY_60","attr":"$.message.order.billing.address.country","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_AREA_CODE_61(input_data):
            ...
            for REQUIRED_MESSAGE_AREA_CODE_61_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_AREA_CODE_61_obj, "$.message.order.billing.address.area_code")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_AREA_CODE_61**: $.message.order.billing.address.area_code must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_AREA_CODE_61","attr":"$.message.order.billing.address.area_code","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_PHONE_63(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_PHONE_63_obj in scope:
                REQUIRED_MESSAGE_PHONE_63_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_PHONE_63_obj, "$.message.order.billing.phone")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_PHONE_63_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_PHONE_63",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_PHONE_63**: $.message.order.billing.phone must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_PHONE_63","attr":"$.message.order.billing.phone","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_EMAIL_64(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_EMAIL_64_obj in scope:
                REQUIRED_MESSAGE_EMAIL_64_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_EMAIL_64_obj, "$.message.order.billing.email")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_EMAIL_64_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_EMAIL_64",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_EMAIL_64**: $.message.order.billing.email must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_EMAIL_64","attr":"$.message.order.billing.email","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_TYPE_68(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_TYPE_68_obj in scope:
                REQUIRED_MESSAGE_TYPE_68_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_TYPE_68_obj, "$.message.order.payment.type")
                enumList = ["ON-ORDER","ON-FULFILLMENT","POST-FULFILLMENT"]

                validate = (validation_utils["are_present"](attr)) and (validation_utils["all_in"](attr, enumList))

                if not validate:
                    del REQUIRED_MESSAGE_TYPE_68_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_TYPE_68",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_TYPE_68**: all of the following sub conditions must be met:
          ...
          - **condition REQUIRED_MESSAGE_TYPE_68.2**: every element of $.message.order.payment.type must be in ["ON-ORDER", "ON-FULFILLMENT", "POST-FULFILLMENT"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_TYPE_68","attr":"$.message.order.payment.type","_RETURN_":"attr are present && attr all in enumList","enumList":["ON-ORDER","ON-FULFILLMENT","POST-FULFILLMENT"]}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_CATEGORY_ID_69(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_CATEGORY_ID_69_obj in scope:
                REQUIRED_MESSAGE_CATEGORY_ID_69_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_CATEGORY_ID_69_obj, "$.message.order['@ondc/org/linked_order'].items[*].category_id")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_CATEGORY_ID_69_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_CATEGORY_ID_69_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_CATEGORY_ID_69",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_CATEGORY_ID_69**: $.message.order['@ondc/org/linked_order'].items[*].category_id must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_CATEGORY_ID_69","attr":"$.message.order['@ondc/org/linked_order'].items[*].category_id","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_NAME_70(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_NAME_70_obj in scope:
                REQUIRED_MESSAGE_NAME_70_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_70_obj, "$.message.order['@ondc/org/linked_order'].items[*].descriptor.name")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_70_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_NAME_70_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_NAME_70",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_NAME_70**: $.message.order['@ondc/org/linked_order'].items[*].descriptor.name must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_NAME_70","attr":"$.message.order['@ondc/org/linked_order'].items[*].descriptor.name","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_VALUE_73(input_data):
            ...
            for REQUIRED_MESSAGE_VALUE_73_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_VALUE_73_obj, "$.message.order['@ondc/org/linked_order'].items[*].quantity.measure.value")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_VALUE_73**: $.message.order['@ondc/org/linked_order'].items[*].quantity.measure.value must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_VALUE_73","attr":"$.message.order['@ondc/org/linked_order'].items[*].quantity.measure.value","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_CURRENCY_74(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_CURRENCY_74_obj in scope:
                REQUIRED_MESSAGE_CURRENCY_74_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_CURRENCY_74_obj, "$.message.order['@ondc/org/linked_order'].items[*].price.currency")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_CURRENCY_74_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_CURRENCY_74_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_CURRENCY_74",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_CURRENCY_74**: $.message.order['@ondc/org/linked_order'].items[*].price.currency must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_CURRENCY_74","attr":"$.message.order['@ondc/org/linked_order'].items[*].price.currency","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_VALUE_75(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_VALUE_75_obj in scope:
                REQUIRED_MESSAGE_VALUE_75_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_VALUE_75_obj, "$.message.order['@ondc/org/linked_order'].items[*].price.value")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_VALUE_75_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_VALUE_75_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_VALUE_75",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_VALUE_75**: $.message.order['@ondc/org/linked_order'].items[*].price.value must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_VALUE_75","attr":"$.message.order['@ondc/org/linked_order'].items[*].price.value","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_NAME_76(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_NAME_76_obj in scope:
                REQUIRED_MESSAGE_NAME_76_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_76_obj, "$.message.order['@ondc/org/linked_order'].provider.descriptor.name")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_76_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_NAME_76_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_NAME_76",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_NAME_76**: $.message.order['@ondc/org/linked_order'].provider.descriptor.name must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_NAME_76","attr":"$.message.order['@ondc/org/linked_order'].provider.descriptor.name","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_NAME_77(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_NAME_77_obj in scope:
                REQUIRED_MESSAGE_NAME_77_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_77_obj, "$.message.order['@ondc/org/linked_order'].provider.address.name")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_77_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_NAME_77_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_NAME_77",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_NAME_77**: $.message.order['@ondc/org/linked_order'].provider.address.name must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_NAME_77","attr":"$.message.order['@ondc/org/linked_order'].provider.address.name","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_BUILDING_78(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_BUILDING_78_obj in scope:
                REQUIRED_MESSAGE_BUILDING_78_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_BUILDING_78_obj, "$.message.order['@ondc/org/linked_order'].provider.address.building")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_BUILDING_78_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_BUILDING_78_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_BUILDING_78",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_BUILDING_78**: $.message.order['@ondc/org/linked_order'].provider.address.building must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_BUILDING_78","attr":"$.message.order['@ondc/org/linked_order'].provider.address.building","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_LOCALITY_79(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_LOCALITY_79_obj in scope:
                REQUIRED_MESSAGE_LOCALITY_79_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_LOCALITY_79_obj, "$.message.order['@ondc/org/linked_order'].provider.address.locality")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_LOCALITY_79_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_LOCALITY_79_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_LOCALITY_79",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_LOCALITY_79**: $.message.order['@ondc/org/linked_order'].provider.address.locality must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_LOCALITY_79","attr":"$.message.order['@ondc/org/linked_order'].provider.address.locality","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_CITY_80(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_CITY_80_obj in scope:
                REQUIRED_MESSAGE_CITY_80_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_CITY_80_obj, "$.message.order['@ondc/org/linked_order'].provider.address.city")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_CITY_80_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_CITY_80_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_CITY_80",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_CITY_80**: $.message.order['@ondc/org/linked_order'].provider.address.city must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_CITY_80","attr":"$.message.order['@ondc/org/linked_order'].provider.address.city","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_STATE_81(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_STATE_81_obj in scope:
                REQUIRED_MESSAGE_STATE_81_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_STATE_81_obj, "$.message.order['@ondc/org/linked_order'].provider.address.state")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_STATE_81_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_STATE_81_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_STATE_81",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_STATE_81**: $.message.order['@ondc/org/linked_order'].provider.address.state must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_STATE_81","attr":"$.message.order['@ondc/org/linked_order'].provider.address.state","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_AREA_CODE_82(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_AREA_CODE_82_obj in scope:
                REQUIRED_MESSAGE_AREA_CODE_82_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_AREA_CODE_82_obj, "$.message.order['@ondc/org/linked_order'].provider.address.area_code")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_AREA_CODE_82_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_AREA_CODE_82_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_AREA_CODE_82",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_AREA_CODE_82**: $.message.order['@ondc/org/linked_order'].provider.address.area_code must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_AREA_CODE_82","attr":"$.message.order['@ondc/org/linked_order'].provider.address.area_code","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_ID_83(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_ID_83_obj in scope:
                REQUIRED_MESSAGE_ID_83_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_ID_83_obj, "$.message.order['@ondc/org/linked_order'].order.id")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_ID_83_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Batch"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_ID_83_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_ID_83",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_ID_83**: $.message.order['@ondc/org/linked_order'].order.id must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_ID_83","attr":"$.message.order['@ondc/org/linked_order'].order.id","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_UNIT_84(input_data):
            ...
            for REQUIRED_MESSAGE_UNIT_84_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_UNIT_84_obj, "$.message.order['@ondc/org/linked_order'].order.weight.unit")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_UNIT_84**: $.message.order['@ondc/org/linked_order'].order.weight.unit must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_UNIT_84","attr":"$.message.order['@ondc/org/linked_order'].order.weight.unit","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_VALUE_85(input_data):
            ...
            for REQUIRED_MESSAGE_VALUE_85_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_VALUE_85_obj, "$.message.order['@ondc/org/linked_order'].order.weight.value")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_VALUE_85**: $.message.order['@ondc/org/linked_order'].order.weight.value must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_VALUE_85","attr":"$.message.order['@ondc/org/linked_order'].order.weight.value","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_UNIT_86(input_data):
            ...
            for REQUIRED_MESSAGE_UNIT_86_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_UNIT_86_obj, "$.message.order['@ondc/org/linked_order'].order.dimensions.length.unit")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_UNIT_86**: $.message.order['@ondc/org/linked_order'].order.dimensions.length.unit must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_UNIT_86","attr":"$.message.order['@ondc/org/linked_order'].order.dimensions.length.unit","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_VALUE_87(input_data):
            ...
            for REQUIRED_MESSAGE_VALUE_87_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_VALUE_87_obj, "$.message.order['@ondc/org/linked_order'].order.dimensions.length.value")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_VALUE_87**: $.message.order['@ondc/org/linked_order'].order.dimensions.length.value must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_VALUE_87","attr":"$.message.order['@ondc/org/linked_order'].order.dimensions.length.value","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_UNIT_88(input_data):
            ...
            for REQUIRED_MESSAGE_UNIT_88_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_UNIT_88_obj, "$.message.order['@ondc/org/linked_order'].order.dimensions.breadth.unit")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_UNIT_88**: $.message.order['@ondc/org/linked_order'].order.dimensions.breadth.unit must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_UNIT_88","attr":"$.message.order['@ondc/org/linked_order'].order.dimensions.breadth.unit","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_VALUE_89(input_data):
            ...
            for REQUIRED_MESSAGE_VALUE_89_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_VALUE_89_obj, "$.message.order['@ondc/org/linked_order'].order.dimensions.breadth.value")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_VALUE_89**: $.message.order['@ondc/org/linked_order'].order.dimensions.breadth.value must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_VALUE_89","attr":"$.message.order['@ondc/org/linked_order'].order.dimensions.breadth.value","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_UNIT_90(input_data):
            ...
            for REQUIRED_MESSAGE_UNIT_90_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_UNIT_90_obj, "$.message.order['@ondc/org/linked_order'].order.dimensions.height.unit")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_UNIT_90**: $.message.order['@ondc/org/linked_order'].order.dimensions.height.unit must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_UNIT_90","attr":"$.message.order['@ondc/org/linked_order'].order.dimensions.height.unit","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_VALUE_91(input_data):
            ...
            for REQUIRED_MESSAGE_VALUE_91_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_VALUE_91_obj, "$.message.order['@ondc/org/linked_order'].order.dimensions.height.value")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_VALUE_91**: $.message.order['@ondc/org/linked_order'].order.dimensions.height.value must be present in the payload
        	...
        	> - **condition B**: at least one element of $.message.order.fulfillments[*].type must be in ["Batch"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_VALUE_91","attr":"$.message.order['@ondc/org/linked_order'].order.dimensions.height.value","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Batch"],"_CONTINUE_":"(fulfillmentType any in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_CREATED_AT_92(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_CREATED_AT_92_obj in scope:
                REQUIRED_MESSAGE_CREATED_AT_92_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_CREATED_AT_92_obj, "$.message.order.created_at")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_CREATED_AT_92_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_CREATED_AT_92",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_CREATED_AT_92**: $.message.order.created_at must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_CREATED_AT_92","attr":"$.message.order.created_at","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_UPDATED_AT_93(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_UPDATED_AT_93_obj in scope:
                REQUIRED_MESSAGE_UPDATED_AT_93_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_UPDATED_AT_93_obj, "$.message.order.updated_at")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_UPDATED_AT_93_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_UPDATED_AT_93",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_UPDATED_AT_93**: $.message.order.updated_at must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_UPDATED_AT_93","attr":"$.message.order.updated_at","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def validate_tag_3(input_data):
            ...
            for validate_tag_3_obj in scope:
                ...
                validTags = ["linked_provider","linked_order","linked_order_item","cod_settlement_detail","fulfill_request","fulfill_response","state","rto_action","rto_verification","shipping_label","weather_check"]
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_3**: every element of $.message.order.fulfillments[*].tags[*].code must be in ["linked_provider", "linked_order", "linked_order_item", "cod_settlement_detail", "fulfill_request", "fulfill_response", "state", "rto_action", "rto_verification", "shipping_label", "weather_check"]
        	...
        	> - **condition B**: $.message.order.fulfillments[*].tags[*].code must **not** be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_3","validTags":["linked_provider","linked_order","linked_order_item","cod_settlement_detail","fulfill_request","fulfill_response","state","rto_action","rto_verification","shipping_label","weather_check"],"tagPath":"$.message.order.fulfillments[*].tags[*].code","_CONTINUE_":"!(tagPath are present)","_RETURN_":"tagPath all in validTags"}
        """
                        }
                    }]
            ...
            for validate_tag_3_linked_order_obj in scope:
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_3_linked_order**: every element of $.message.order.fulfillments[*].tags[?(@.code=='linked_order')].list[*].code must be in ["id", "prep_time", "cod_order", "collection_amount", "currency", "declared_value", "weight_unit", "weight_value", "dim_unit", "length", "breadth", "height", "shipment_type"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_3_linked_order","_SCOPE_":"$.message.order.fulfillments[*].tags[?(@.code=='linked_order')]","subTags":"$.list[*].code","validValues":["id","prep_time","cod_order","collection_amount","currency","declared_value","weight_unit","weight_value","dim_unit","length","breadth","height","shipment_type"],"_RETURN_":"subTags all in validValues"}
        """
                        }
                    }]
            ...
            for validate_tag_3_linked_order_item_obj in scope:
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_3_linked_order_item**: every element of $.message.order.fulfillments[*].tags[?(@.code=='linked_order_item')].list[*].code must be in ["category", "name", "currency", "value", "quantity", "weight_unit", "weight_value", "hsn_code", "ebn_exempt"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_3_linked_order_item","_SCOPE_":"$.message.order.fulfillments[*].tags[?(@.code=='linked_order_item')]","subTags":"$.list[*].code","validValues":["category","name","currency","value","quantity","weight_unit","weight_value","hsn_code","ebn_exempt"],"_RETURN_":"subTags all in validValues"}
        """
                        }
                    }]
        ...
        def validate_tag_3_shipping_label(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$.message.order.fulfillments[*].tags[?(@.code=='shipping_label')]")
            sub_results = []
            valid = True

            for validate_tag_3_shipping_label_obj in scope:
                validate_tag_3_shipping_label_obj["_EXTERNAL"] = input_data["external_data"]
                subTags = payload_utils["get_json_path"](validate_tag_3_shipping_label_obj, "$.list[*].code")
                validValues = ["type","url"]

                validate = validation_utils["all_in"](subTags, validValues)

                if not validate:
                    del validate_tag_3_shipping_label_obj["_EXTERNAL"]
                    return [{
                        "test_name": "validate_tag_3_shipping_label",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_3_shipping_label**: every element of $.message.order.fulfillments[*].tags[?(@.code=='shipping_label')].list[*].code must be in ["type", "url"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_3_shipping_label","_SCOPE_":"$.message.order.fulfillments[*].tags[?(@.code=='shipping_label')]","subTags":"$.list[*].code","validValues":["type","url"],"_RETURN_":"subTags all in validValues"}
        """
                        }
                    }]
        ...
        def validate_tag_3_weather_check(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$.message.order.fulfillments[*].tags[?(@.code=='weather_check')]")
            sub_results = []
            valid = True

            for validate_tag_3_weather_check_obj in scope:
                validate_tag_3_weather_check_obj["_EXTERNAL"] = input_data["external_data"]
                subTags = payload_utils["get_json_path"](validate_tag_3_weather_check_obj, "$.list[*].code")
                validValues = ["raining"]

                validate = validation_utils["all_in"](subTags, validValues)

                if not validate:
                    del validate_tag_3_weather_check_obj["_EXTERNAL"]
                    return [{
                        "test_name": "validate_tag_3_weather_check",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_3_weather_check**: every element of $.message.order.fulfillments[*].tags[?(@.code=='weather_check')].list[*].code must be in ["raining"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_3_weather_check","_SCOPE_":"$.message.order.fulfillments[*].tags[?(@.code=='weather_check')]","subTags":"$.list[*].code","validValues":["raining"],"_RETURN_":"subTags all in validValues"}
        """
                        }
                    }]
        ...
        def validate_tag_4(input_data):
            ...
            for validate_tag_4_obj in scope:
                ...
                validTags = ["bpp_terms","bap_terms","lbnp_sla_terms"]
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_4**: every element of $.message.order.tags[*].code must be in ["bpp_terms", "bap_terms", "lbnp_sla_terms"]
        	...
        	> - **condition B**: $.message.order.tags[*].code must **not** be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_4","validTags":["bpp_terms","bap_terms","lbnp_sla_terms"],"tagPath":"$.message.order.tags[*].code","_CONTINUE_":"!(tagPath are present)","_RETURN_":"tagPath all in validTags"}
        """
                        }
                    }]
        ...
        def validate_tag_4_bpp_terms(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$.message.order.tags[?(@.code=='bpp_terms')]")
            sub_results = []
            valid = True

            for validate_tag_4_bpp_terms_obj in scope:
                validate_tag_4_bpp_terms_obj["_EXTERNAL"] = input_data["external_data"]
                subTags = payload_utils["get_json_path"](validate_tag_4_bpp_terms_obj, "$.list[*].code")
                validValues = ["static_terms","phone","np_tax_type","max_liability","max_liability_cap","mandatory_arbitration","mandatory_arbitration","court_jurisdiction","delay_interest"]

                validate = validation_utils["all_in"](subTags, validValues)

                if not validate:
                    del validate_tag_4_bpp_terms_obj["_EXTERNAL"]
                    return [{
                        "test_name": "validate_tag_4_bpp_terms",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_4_bpp_terms**: every element of $.message.order.tags[?(@.code=='bpp_terms')].list[*].code must be in ["static_terms", "phone", "np_tax_type", "max_liability", "max_liability_cap", "mandatory_arbitration", "mandatory_arbitration", "court_jurisdiction", "delay_interest"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_4_bpp_terms","_SCOPE_":"$.message.order.tags[?(@.code=='bpp_terms')]","subTags":"$.list[*].code","validValues":["static_terms","phone","np_tax_type","max_liability","max_liability_cap","mandatory_arbitration","mandatory_arbitration","court_jurisdiction","delay_interest"],"_RETURN_":"subTags all in validValues"}
        """
                        }
                    }]
            ...
            for validate_tag_4_bap_terms_obj in scope:
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_4_bap_terms**: every element of $.message.order.tags[?(@.code=='bap_terms')].list[*].code must be in ["accept_bpp_terms"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_4_bap_terms","_SCOPE_":"$.message.order.tags[?(@.code=='bap_terms')]","subTags":"$.list[*].code","validValues":["accept_bpp_terms"],"_RETURN_":"subTags all in validValues"}
        """
                        }
                    }]
        ...
        def on_confirm_fulfillment_id_validations(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for on_confirm_fulfillment_id_validations_obj in scope:
                on_confirm_fulfillment_id_validations_obj["_EXTERNAL"] = input_data["external_data"]
        ...
        test_functions = [
            ...
            REQUIRED_MESSAGE_GPS_45,
            REQUIRED_MESSAGE_NAME_46,
            REQUIRED_MESSAGE_BUILDING_47,
            REQUIRED_MESSAGE_LOCALITY_48,
            REQUIRED_MESSAGE_CITY_49,
            REQUIRED_MESSAGE_STATE_50,
            REQUIRED_MESSAGE_COUNTRY_51,
            REQUIRED_MESSAGE_AREA_CODE_52,
            REQUIRED_MESSAGE_PHONE_53,
            REQUIRED_MESSAGE_NAME_54,
            REQUIRED_MESSAGE_NAME_55,
            REQUIRED_MESSAGE_BUILDING_56,
            REQUIRED_MESSAGE_LOCALITY_57,
            REQUIRED_MESSAGE_CITY_58,
            REQUIRED_MESSAGE_STATE_59,
            REQUIRED_MESSAGE_COUNTRY_60,
            REQUIRED_MESSAGE_AREA_CODE_61,
            REQUIRED_MESSAGE_TAX_NUMBER,
            REQUIRED_MESSAGE_PHONE_63,
            REQUIRED_MESSAGE_EMAIL_64,
            REQUIRED_MESSAGE_CREATED_AT,
            REQUIRED_MESSAGE_UPDATED_AT,
            REQUIRED_MESSAGE_COLLECTED_BY,
            REQUIRED_MESSAGE_TYPE_68,
            REQUIRED_MESSAGE_CATEGORY_ID_69,
            REQUIRED_MESSAGE_NAME_70,
            ...
            REQUIRED_MESSAGE_BUILDING_78,
            REQUIRED_MESSAGE_LOCALITY_79,
            REQUIRED_MESSAGE_CITY_80,
            REQUIRED_MESSAGE_STATE_81,
            REQUIRED_MESSAGE_AREA_CODE_82,
            REQUIRED_MESSAGE_ID_83,
            ...
            REQUIRED_MESSAGE_CREATED_AT_92,
            REQUIRED_MESSAGE_UPDATED_AT_93,
