    ...
    for init_validations_obj in scope:
        ...
        def REQUIRED_MESSAGE_ID(input_data):
            ...
            for REQUIRED_MESSAGE_ID_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_ID_obj, "$.message.order.provider.id")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_ID**: $.message.order.provider.id must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_ID","attr":"$.message.order.provider.id","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_ID_15(input_data):
            ...
            for REQUIRED_MESSAGE_ID_15_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_ID_15_obj, "$.message.order.items[*].id")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_ID_15**: $.message.order.items[*].id must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_ID_15","attr":"$.message.order.items[*].id","_RETURN_":"attr are present"}
        """
                        }
                    }]
            ...
            for REQUIRED_MESSAGE_CATEGORY_ID_obj in scope:
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_CATEGORY_ID**: $.message.order.items[*].category_id must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_CATEGORY_ID","attr":"$.message.order.items[*].category_id","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_ID_17(input_data):
            ...
            for REQUIRED_MESSAGE_ID_17_obj in scope:
                ...
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_ID_17_obj, "$.message.order.fulfillments[*].id")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_ID_17**: $.message.order.fulfillments[*].id must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_ID_17","attr":"$.message.order.fulfillments[*].id","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_GPS_29(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_GPS_29_obj in scope:
                REQUIRED_MESSAGE_GPS_29_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_GPS_29_obj, "$.message.order.fulfillments[*].end.location.gps")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_GPS_29_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_GPS_29_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_GPS_29",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_GPS_29**: $.message.order.fulfillments[*].end.location.gps must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_GPS_29","attr":"$.message.order.fulfillments[*].end.location.gps","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_NAME_30(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_NAME_30_obj in scope:
                REQUIRED_MESSAGE_NAME_30_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_30_obj, "$.message.order.fulfillments[*].end.location.address.name")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_30_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_NAME_30_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_NAME_30",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_NAME_30**: $.message.order.fulfillments[*].end.location.address.name must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_NAME_30","attr":"$.message.order.fulfillments[*].end.location.address.name","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_BUILDING_31(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_BUILDING_31_obj in scope:
                REQUIRED_MESSAGE_BUILDING_31_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_BUILDING_31_obj, "$.message.order.fulfillments[*].end.location.address.building")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_BUILDING_31_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_BUILDING_31_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_BUILDING_31",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_BUILDING_31**: $.message.order.fulfillments[*].end.location.address.building must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_BUILDING_31","attr":"$.message.order.fulfillments[*].end.location.address.building","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_LOCALITY_32(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_LOCALITY_32_obj in scope:
                REQUIRED_MESSAGE_LOCALITY_32_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_LOCALITY_32_obj, "$.message.order.fulfillments[*].end.location.address.locality")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_LOCALITY_32_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_LOCALITY_32_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_LOCALITY_32",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_LOCALITY_32**: $.message.order.fulfillments[*].end.location.address.locality must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_LOCALITY_32","attr":"$.message.order.fulfillments[*].end.location.address.locality","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_CITY_33(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_CITY_33_obj in scope:
                REQUIRED_MESSAGE_CITY_33_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_CITY_33_obj, "$.message.order.fulfillments[*].end.location.address.city")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_CITY_33_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_CITY_33_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_CITY_33",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_CITY_33**: $.message.order.fulfillments[*].end.location.address.city must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_CITY_33","attr":"$.message.order.fulfillments[*].end.location.address.city","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_STATE_34(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_STATE_34_obj in scope:
                REQUIRED_MESSAGE_STATE_34_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_STATE_34_obj, "$.message.order.fulfillments[*].end.location.address.state")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_STATE_34_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_STATE_34_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_STATE_34",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_STATE_34**: $.message.order.fulfillments[*].end.location.address.state must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_STATE_34","attr":"$.message.order.fulfillments[*].end.location.address.state","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_COUNTRY_35(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_COUNTRY_35_obj in scope:
                REQUIRED_MESSAGE_COUNTRY_35_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_COUNTRY_35_obj, "$.message.order.fulfillments[*].end.location.address.country")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_COUNTRY_35_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_COUNTRY_35_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_COUNTRY_35",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_COUNTRY_35**: $.message.order.fulfillments[*].end.location.address.country must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_COUNTRY_35","attr":"$.message.order.fulfillments[*].end.location.address.country","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_AREA_CODE_36(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_AREA_CODE_36_obj in scope:
                REQUIRED_MESSAGE_AREA_CODE_36_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_AREA_CODE_36_obj, "$.message.order.fulfillments[*].end.location.address.area_code")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_AREA_CODE_36_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_AREA_CODE_36_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_AREA_CODE_36",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_AREA_CODE_36**: $.message.order.fulfillments[*].end.location.address.area_code must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_AREA_CODE_36","attr":"$.message.order.fulfillments[*].end.location.address.area_code","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_PHONE_37(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_PHONE_37_obj in scope:
                REQUIRED_MESSAGE_PHONE_37_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_PHONE_37_obj, "$.message.order.fulfillments[*].end.contact.phone")
                fulfillmentType = payload_utils["get_json_path"](REQUIRED_MESSAGE_PHONE_37_obj, "$.message.order.fulfillments[*].type")
                fulType = ["Delivery"]
                ...
                if not validate:
                    del REQUIRED_MESSAGE_PHONE_37_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_PHONE_37",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_PHONE_37**: $.message.order.fulfillments[*].end.contact.phone must be present in the payload
        	...
        	> - **condition B**: every element of $.message.order.fulfillments[*].type must **not** be in ["Delivery"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_PHONE_37","attr":"$.message.order.fulfillments[*].end.contact.phone","fulfillmentType":"$.message.order.fulfillments[*].type","fulType":["Delivery"],"_CONTINUE_":"!(fulfillmentType all in fulType)","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_NAME_38(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_NAME_38_obj in scope:
                REQUIRED_MESSAGE_NAME_38_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_38_obj, "$.message.order.billing.name")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_NAME_38_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_NAME_38",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_NAME_38**: $.message.order.billing.name must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_NAME_38","attr":"$.message.order.billing.name","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_NAME_39(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_NAME_39_obj in scope:
                REQUIRED_MESSAGE_NAME_39_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_NAME_39_obj, "$.message.order.billing.address.name")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_NAME_39_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_NAME_39",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_NAME_39**: $.message.order.billing.address.name must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_NAME_39","attr":"$.message.order.billing.address.name","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_BUILDING_40(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_BUILDING_40_obj in scope:
                REQUIRED_MESSAGE_BUILDING_40_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_BUILDING_40_obj, "$.message.order.billing.address.building")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_BUILDING_40_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_BUILDING_40",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_BUILDING_40**: $.message.order.billing.address.building must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_BUILDING_40","attr":"$.message.order.billing.address.building","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_LOCALITY_41(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_LOCALITY_41_obj in scope:
                REQUIRED_MESSAGE_LOCALITY_41_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_LOCALITY_41_obj, "$.message.order.billing.address.locality")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_LOCALITY_41_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_LOCALITY_41",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_LOCALITY_41**: $.message.order.billing.address.locality must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_LOCALITY_41","attr":"$.message.order.billing.address.locality","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_CITY_42(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_CITY_42_obj in scope:
                REQUIRED_MESSAGE_CITY_42_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_CITY_42_obj, "$.message.order.billing.address.city")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_CITY_42_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_CITY_42",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_CITY_42**: $.message.order.billing.address.city must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_CITY_42","attr":"$.message.order.billing.address.city","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_STATE_43(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_STATE_43_obj in scope:
                REQUIRED_MESSAGE_STATE_43_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_STATE_43_obj, "$.message.order.billing.address.state")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_STATE_43_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_STATE_43",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_STATE_43**: $.message.order.billing.address.state must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_STATE_43","attr":"$.message.order.billing.address.state","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_COUNTRY_44(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_COUNTRY_44_obj in scope:
                REQUIRED_MESSAGE_COUNTRY_44_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_COUNTRY_44_obj, "$.message.order.billing.address.country")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_COUNTRY_44_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_COUNTRY_44",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_COUNTRY_44**: $.message.order.billing.address.country must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_COUNTRY_44","attr":"$.message.order.billing.address.country","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_AREA_CODE_45(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_AREA_CODE_45_obj in scope:
                REQUIRED_MESSAGE_AREA_CODE_45_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_AREA_CODE_45_obj, "$.message.order.billing.address.area_code")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_AREA_CODE_45_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_AREA_CODE_45",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_AREA_CODE_45**: $.message.order.billing.address.area_code must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_AREA_CODE_45","attr":"$.message.order.billing.address.area_code","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_PHONE_47(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_PHONE_47_obj in scope:
                REQUIRED_MESSAGE_PHONE_47_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_PHONE_47_obj, "$.message.order.billing.phone")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_PHONE_47_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_PHONE_47",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_PHONE_47**: $.message.order.billing.phone must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_PHONE_47","attr":"$.message.order.billing.phone","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_EMAIL_48(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_EMAIL_48_obj in scope:
                REQUIRED_MESSAGE_EMAIL_48_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_EMAIL_48_obj, "$.message.order.billing.email")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_EMAIL_48_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_EMAIL_48",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_EMAIL_48**: $.message.order.billing.email must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_EMAIL_48","attr":"$.message.order.billing.email","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def REQUIRED_MESSAGE_TYPE_52(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_TYPE_52_obj in scope:
                REQUIRED_MESSAGE_TYPE_52_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_TYPE_52_obj, "$.message.order.payment.type")
                enumList = ["ON-ORDER","ON-FULFILLMENT","POST-FULFILLMENT"]

                validate = (validation_utils["are_present"](attr)) and (validation_utils["all_in"](attr, enumList))

                if not validate:
                    del REQUIRED_MESSAGE_TYPE_52_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_TYPE_52",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_TYPE_52**: all of the following sub conditions must be met:
          ...
          - **condition REQUIRED_MESSAGE_TYPE_52.2**: every element of $.message.order.payment.type must be in ["ON-ORDER", "ON-FULFILLMENT", "POST-FULFILLMENT"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_TYPE_52","attr":"$.message.order.payment.type","_RETURN_":"attr are present && attr all in enumList","enumList":["ON-ORDER","ON-FULFILLMENT","POST-FULFILLMENT"]}
        """
                        }
                    }]
        ...
        def validate_tag_2(input_data):
            ...
            for validate_tag_2_obj in scope:
                ...
                validTags = ["linked_provider","fulfill_request","fulfill_response","linked_order"]
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_2**: every element of $.message.order.fulfillments[*].tags[*].code must be in ["linked_provider", "fulfill_request", "fulfill_response", "linked_order"]
        	...
        	> - **condition B**: $.message.order.fulfillments[*].tags[*].code must **not** be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_2","validTags":["linked_provider","fulfill_request","fulfill_response","linked_order"],"tagPath":"$.message.order.fulfillments[*].tags[*].code","_CONTINUE_":"!(tagPath are present)","_RETURN_":"tagPath all in validTags"}
        """
                        }
                    }]
        ...
        def validate_tag_2_linked_order(input_data):
            ...
            for validate_tag_2_linked_order_obj in scope:
                ...
                validValues = ["id","cod_order","collection_amount","currency","declared_value","category","weight_unit","weight_value","dim_unit","length","breadth","height"]
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_2_linked_order**: every element of $.message.order.fulfillments[*].tags[?(@.code=='linked_order')].list[*].code must be in ["id", "cod_order", "collection_amount", "currency", "declared_value", "category", "weight_unit", "weight_value", "dim_unit", "length", "breadth", "height"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_2_linked_order","_SCOPE_":"$.message.order.fulfillments[*].tags[?(@.code=='linked_order')]","subTags":"$.list[*].code","validValues":["id","cod_order","collection_amount","currency","declared_value","category","weight_unit","weight_value","dim_unit","length","breadth","height"],"_RETURN_":"subTags all in validValues"}
        """
                        }
                    }]
        ...
        def validate_tag_3(input_data):
            ...
            for validate_tag_3_obj in scope:
                ...
                validTags = ["type"]
                tagPath = payload_utils["get_json_path"](validate_tag_3_obj, "$.message.order.items[*].tags[*].code")
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_3**: every element of $.message.order.items[*].tags[*].code must be in ["type"]
        	...
        	> - **condition B**: $.message.order.items[*].tags[*].code must **not** be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_3","validTags":["type"],"tagPath":"$.message.order.items[*].tags[*].code","_CONTINUE_":"!(tagPath are present)","_RETURN_":"tagPath all in validTags"}
        """
                        }
                    }]
        ...
        def validate_tag_3_type(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$.message.order.items[*].tags[?(@.code=='type')]")
            sub_results = []
            valid = True

            for validate_tag_3_type_obj in scope:
                validate_tag_3_type_obj["_EXTERNAL"] = input_data["external_data"]
                subTags = payload_utils["get_json_path"](validate_tag_3_type_obj, "$.list[*].code")
                validValues = ["type"]

                validate = validation_utils["all_in"](subTags, validValues)

                if not validate:
                    del validate_tag_3_type_obj["_EXTERNAL"]
                    return [{
                        "test_name": "validate_tag_3_type",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_3_type**: every element of $.message.order.items[*].tags[?(@.code=='type')].list[*].code must be in ["type"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_3_type","_SCOPE_":"$.message.order.items[*].tags[?(@.code=='type')]","subTags":"$.list[*].code","validValues":["type"],"_RETURN_":"subTags all in validValues"}
        """
                        }
                    }]
        ...
        def validate_tag_4(input_data):
            ...
            for validate_tag_4_obj in scope:
                ...
                if not validate:
                    ...
                    return [{
                        ...
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition validate_tag_4**: every element of $.message.order.tags[*].code must be in ["bap_terms"]
        	...
        	> - **condition B**: $.message.order.tags[*].code must **not** be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_tag_4","validTags":["bap_terms"],"tagPath":"$.message.order.tags[*].code","_CONTINUE_":"!(tagPath are present)","_RETURN_":"tagPath all in validTags"}
        """
                        }
                    }]
        ...
        def init_fulfillment_id_validations(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for init_fulfillment_id_validations_obj in scope:
                init_fulfillment_id_validations_obj["_EXTERNAL"] = input_data["external_data"]
