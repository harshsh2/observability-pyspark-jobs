...
def cancel_validations(input_data):
    ...
    for cancel_validations_obj in scope:
        ...
        def REQUIRED_MESSAGE_CANCELLATION_REASON_ID(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for REQUIRED_MESSAGE_CANCELLATION_REASON_ID_obj in scope:
                REQUIRED_MESSAGE_CANCELLATION_REASON_ID_obj["_EXTERNAL"] = input_data["external_data"]
                attr = payload_utils["get_json_path"](REQUIRED_MESSAGE_CANCELLATION_REASON_ID_obj, "$.message.cancellation_reason_id")

                validate = validation_utils["are_present"](attr)

                if not validate:
                    del REQUIRED_MESSAGE_CANCELLATION_REASON_ID_obj["_EXTERNAL"]
                    return [{
                        "test_name": "REQUIRED_MESSAGE_CANCELLATION_REASON_ID",
                        "valid": False,
                        "code": 30000,
                        "description": r"""- **condition REQUIRED_MESSAGE_CANCELLATION_REASON_ID**: $.message.cancellation_reason_id must be present in the payload""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"REQUIRED_MESSAGE_CANCELLATION_REASON_ID","attr":"$.message.cancellation_reason_id","_RETURN_":"attr are present"}
        """
                        }
                    }]
        ...
        def validate_cancellation_codes_log10(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for validate_cancellation_codes_log10_obj in scope:
                validate_cancellation_codes_log10_obj["_EXTERNAL"] = input_data["external_data"]
                domain = payload_utils["get_json_path"](validate_cancellation_codes_log10_obj, "$.context.domain")
                p2pCancellationCodes = ["004","005","007","150","151","152","153","154","155","156","175","996","994"]
                domainList = ["ONDC:LOG10"]
                cancellationId = payload_utils["get_json_path"](validate_cancellation_codes_log10_obj, "$.message.cancellation_reason_id")

                skip_check = not (validation_utils["all_in"](domain, domainList))
                if skip_check:
                    continue

                validate = validation_utils["all_in"](cancellationId, p2pCancellationCodes)

                if not validate:
                    del validate_cancellation_codes_log10_obj["_EXTERNAL"]
                    return [{
                        "test_name": "validate_cancellation_codes_log10",
                        "valid": False,
                        "code": 30000,
                        "description": r"""Appropriate cancellation code should be used by LBNP
        	...
        	> - **condition B**: every element of $.context.domain must **not** be in ["ONDC:LOG10"]""",
                        "_debug_info": {
                            "fed_config": r"""
        {"_NAME_":"validate_cancellation_codes_log10","_DESCRIPTION_":"Appropriate cancellation code should be used by LBNP","domain":"$.context.domain","p2pCancellationCodes":["004","005","007","150","151","152","153","154","155","156","175","996","994"],"domainList":["ONDC:LOG10"],"cancellationId":"$.message.cancellation_reason_id","_CONTINUE_":"!(domain all in domainList)","_RETURN_":"cancellationId all in p2pCancellationCodes"}
        """
                        }
                    }]
        ...
        def validate_cancellation_codes_log11(input_data):
            scope = payload_utils["get_json_path"](input_data["payload"], "$")
            sub_results = []
            valid = True

            for validate_cancellation_codes_log11_obj in scope:
                validate_cancellation_codes_log11_obj["_EXTERNAL"] = input_data["external_data"]
                domain = payload_utils["get_json_path"](validate_cancellation_codes_log11_obj, "$.context.domain")
                p2h2pCancellationCodes = ["004","005","007","250","251","252","275","996","994"]
                domainList = ["ONDC:LOG11"]
                cancellationId = payload_utils["get_json_path"](validate_cancellation_codes_log11_obj, "$.message.cancellation_reason_id")

                skip_check = not (validation_utils["all_in"](domain, domainList))
                if skip_check:
                    continue

                validate = validation_utils["all_in"](cancellationId, p2h2pCancellationCodes)

                if not validate:
                    del validate_cancellation_codes_log11_obj["_EXTERNAL"]
                    return [{
                        "test_name": "validate_cancellation_codes_log11",
                        "valid": False,
                        "code": 30000,
                        "description": r"""Appropriate cancellation code should be used by LBNP
